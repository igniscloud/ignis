//! Platform-specific host implementations for Ember.
//!
...
impl HostBindings for SqliteHost {
    ...
    fn add_to_linker<T>(
        ...
        sqlite_bindings::wkr::platform::sqlite::add_to_linker::<T, SqliteImports>(linker, get)
    }
}

impl sqlite_bindings::wkr::platform::sqlite::Host for SqliteHost {
    ...
    fn transaction(
        &mut self,
        statements: Vec<sqlite_bindings::wkr::platform::sqlite::Statement>,
    ...
    fn query(
        ...
        sql: String,
        params: Vec<String>,
    ) -> std::result::Result<sqlite_bindings::wkr::platform::sqlite::QueryResult, String> {
        let connection = self.open_connection()?;
            ...
            .map_err(|error| format!("sqlite iterating rows failed: {error}"))?
        {
            ...
            values.push(sqlite_bindings::wkr::platform::sqlite::Row { values: row_values });
        }
        Ok(sqlite_bindings::wkr::platform::sqlite::QueryResult {
            columns,
            rows: values,
        })
    }
    ...
    fn query_typed(
        ...
        sql: String,
        params: Vec<String>,
    ) -> std::result::Result<sqlite_bindings::wkr::platform::sqlite::TypedQueryResult, String> {
        let connection = self.open_connection()?;
        let mut statement = connection
            .prepare(&sql)
            .map_err(|error| format!("sqlite prepare failed: {error}"))?;
            ...
            .map_err(|error| format!("sqlite iterating rows failed: {error}"))?
        {
            ...
            values.push(sqlite_bindings::wkr::platform::sqlite::TypedRow { values: row_values });
        }
        Ok(sqlite_bindings::wkr::platform::sqlite::TypedQueryResult {
            columns,
            rows: values,
        })
    }
...
fn sqlite_database_path(manifest: &LoadedManifest) -> PathBuf {
    if let Some(path) = manifest.manifest.env.get("WKR_SQLITE_PATH") {
...
fn sqlite_value_to_typed(
    row: &rusqlite::Row<'_>,
    index: usize,
) -> std::result::Result<sqlite_bindings::wkr::platform::sqlite::SqliteValue, String> {
    let value = row
        .get_ref(index)
        .map_err(|error| format!("reading sqlite column {index} failed: {error}"))?;
    Ok(match value {
        rusqlite::types::ValueRef::Null => {
            sqlite_bindings::wkr::platform::sqlite::SqliteValue::Null
        }
        rusqlite::types::ValueRef::Integer(value) => {
            sqlite_bindings::wkr::platform::sqlite::SqliteValue::Integer(value)
        }
        rusqlite::types::ValueRef::Real(value) => {
            sqlite_bindings::wkr::platform::sqlite::SqliteValue::Real(value)
        }
        rusqlite::types::ValueRef::Text(value) => {
            sqlite_bindings::wkr::platform::sqlite::SqliteValue::Text(
                String::from_utf8_lossy(value).into_owned(),
            )
        }
        rusqlite::types::ValueRef::Blob(value) => {
            sqlite_bindings::wkr::platform::sqlite::SqliteValue::Blob(value.to_vec())
        }
    })
}

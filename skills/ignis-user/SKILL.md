---
name: ignis-user
description: Use for people building Ignis workers with ignis-cli, ignis-sdk, worker.toml, local dev, SQLite, secrets, and igniscloud publish/deploy flows.
---
...
- 不要猜测 `worker.toml` 字段、CLI 命令名、`ignis-sdk` 方法或 secret 约定。
- 本地开发优先使用：`ignis init -> ignis build -> ignis dev`。
- 云端交互优先使用：`ignis login -> ignis app publish -> ignis app deploy`。
...
- 配置 `worker.toml` 里的 env、secret、SQLite、network、igniscloud
